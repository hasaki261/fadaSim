...
**Chance.js**, **Piscina.js**, **Math.js** (and **d3-array**) were used to keep the code cleaner, but you can achieve the same result natively using `Math` functions and Workers Threads module. **Inquire.js** is also used for the CLI.
...
### Math
Now the code has math validation. Math results are more precise and fast but simulation results are good to have a cross-validation.You can do only math using the fast argument `node main fast`
> Math results are more precise, but how much precise depends on the "weight" of the simulation: 
The **max** variation can be calculated using $\sqrt(0.25/weight)$, this value is what appears in 'Fast 10x - __Max ± 0.5%__' for example.
---
